...
```python
import heapq             # 우선순위 큐를 쓰기 위함

def solution(jobs):
    jobs.sort()          # 먼저 들어오는 작업기준으로 정렬하기
    answer = 0           # 총 일 한 시간 계산
    present = 0          # 현재시간
    idx = 0
    cnt = 0
    heap = []
    while cnt < len(jobs):  # 일이 다 채워질때까지 돌린다.
        if heap:                                # 할 수 있는 일이 있으면
            time, start = heapq.heappop(heap)   
            answer += present - start + time
            present += time
            cnt += 1
        else:
            present = jobs[idx][0] # 할 수 있는 일이 없으면 가장 가까이있는 일을 하게끔 시간설정
        while 1:
            if idx == len(jobs):   # 모든 일이 heap에 들어가면 더이상 진행 하지 않는다.
                break
            if present >= jobs[idx][0]:  # 지금 현재 할 수 있는 일 모두 heap에 넣기
                heapq.heappush(heap, [jobs[idx][1], jobs[idx][0]])
                idx += 1
            else:
                break
    return answer // len(jobs)  # 총 일한 시간 // 총 일의 갯수
```
