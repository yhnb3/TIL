...
```python
# 이번문제는 sort하는 함수에 순차적인 sorting 조건을 부여할수 있나가 관건 이었던 문제 였습니다.
def solution(files):
    sorting_files = [[] for i in range(len(files))]
    for idx, i in enumerate(files):
        temp = True
        t_idx = 0
        for jdx, j in enumerate(i):
            if j.isdigit() and temp==True:
                sorting_files[idx].append(i[:jdx].lower())
                temp = False
                t_idx = jdx
            elif not j.isdigit() and temp == False:
                sorting_files[idx].append(int(i[t_idx:jdx]))
                sorting_files[idx].append(i[jdx:].lower())
                sorting_files[idx].append(idx)
                break
        else:
            sorting_files[idx].append(int(i[t_idx:]))
            sorting_files[idx].append("")
            sorting_files[idx].append(idx)
    sorting_files.sort(key = lambda word : (word[0].lower(), word[1]))
    answer =  []
    for idx, i in enumerate(sorting_files):
