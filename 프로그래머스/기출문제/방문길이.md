...
```python
def check_road(road_map, ddir, location, answer):
    if ddir == "L":
        if location[0] - 1 >= 0:
            if [location[1], "R"] not in road_map[location[0] - 1] and [location[1], "L"] not in road_map[location[0]]:
                answer += 1
                road_map[location[0]].append([location[1], "L"])
            location[0] -= 1
    elif ddir == "R":
        if location[0] + 1 <= 10:
            if [location[1], "R"] not in road_map[location[0]] and [location[1], "L"] not in road_map[location[0] + 1]:
                answer += 1
                road_map[location[0]].append([location[1], "R"])
            location[0] += 1
    elif ddir == "U":
        if location[1] + 1 <= 10:
            if [location[0], "U"] not in road_map[location[1]] and [location[0], "D"] not in road_map[location[1] + 1]:
                answer += 1
                road_map[location[1]].append([location[0], "U"])
            location[1] += 1
    elif ddir == "D":
        if location[1] - 1 >= 0:
            if [location[0], "D"] not in road_map[location[1]] and [location[0], "U"] not in road_map[location[1] - 1]:
                answer += 1
                road_map[location[1]].append([location[0], "D"])
            location[1] -= 1
    return answer, location

def solution(dirs):
    road_map = [[]for i in range(11)]
    location = [5, 5]
    answer = 0
    for ddir in dirs:
        answer, location = check_road(road_map, ddir, location, answer)
    return answer
```
