...
def check_id(u_id, b_id):
    if len(u_id) != len(b_id):
        return False
    for idx, w in enumerate(b_id):
        if w != "*":
            if u_id[idx] != w:
                return False
    return True
...
def solution(user_id, banned_id):
    ...
    visit = []
    nums = len(banned_id)
    permus = itertools.permutations(range(0, len(user_id)), nums)
    for permu in permus:
        b = sorted(permu)
        if b in visit:
            continue
        for idx, num in enumerate(permu):
            if not check_id(user_id[num], banned_id[idx]):
                break
        else:
            visit.append(b)
            answer += 1
